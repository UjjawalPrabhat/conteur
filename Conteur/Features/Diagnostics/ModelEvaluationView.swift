    ...
    private func headline(_ summary: EvaluationSummary) -> some View {
        Section("Overall") {
            ...
            row(
                ...
                note: "\(summary.failed(.refused).count) refused · \(summary.failed(.contextExceeded).count) filled the window · \(summary.failed(.other).count) other"
            )
            if let elapsed = model.elapsed {
                row(
                    "Took",
                    elapsed.formatted(.units(allowed: [.minutes, .seconds])),
                    note: "one call per event, plus one for the point"
                )
            }
        }
    }
